...
class EditBookingDatesView(View):
    ...
    def get(self, request, pk):
        booking = Booking.objects.get(id=pk)
        booking_dates_form = BookingDatesForm(prefix="booking", instance=booking)
        context = {
    ...
    def post(self, request, pk):
        booking = Booking.objects.get(id=pk)
        booking_dates_form = BookingDatesForm(
            request.POST, prefix="booking", instance=booking
        )

        if booking_dates_form.is_valid():
            checkin = booking_dates_form.cleaned_data["checkin"]
            checkout = booking_dates_form.cleaned_data["checkout"]
        ...
        context = {"booking_dates_form": booking_dates_form}
        return render(request, "edit_booking_dates.html", context)
