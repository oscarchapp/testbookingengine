...
class RoomsView(View):
    def get(self, request):
        # Obtain the form search value
        search_value = request.GET.get("search", "")
        print(type(search_value))
        # Filter the rooms by the search value if it exists
        if search_value:
            rooms = Room.objects.filter(name__icontains=search_value).values(
                "name", "room_type__name", "id"
            )
        else:
            # renders the default list of rooms
            rooms = Room.objects.all().values("name", "room_type__name", "id")

        context = {
            "rooms": rooms,
            "search_value": search_value,  # Give the search value to the template
        }
        # print(context)
