...
class EditBookingCustomerView(View):
...
class RoomsSearchView(View):
    def get(self, request):
        # renders a list of rooms
        query = request.GET.dict()
        if not "filter" in query:
            return redirect("/")
        rooms = Room.objects.filter(Q(name__icontains=query["filter"])).values(
...
class EditBookingView(View):
    ...
    def get(self, request, pk):
        ...
        booking_edit_form = BookingEditForm(prefix="booking", instance=booking)
        context = {"form": booking_edit_form, "booking": booking}
    ...
    def post(self, request, pk):
        ...
        existing_bookings = (
            Booking.objects.filter(
                Q(
                    Q(
                        checkin__gte=query["booking-checkin"],
                        checkin__lte=query["booking-checkout"],
                    )
                    | Q(
                        checkout__gte=query["booking-checkin"],
                        checkout__lte=query["booking-checkout"],
                    )
                ),
                room=booking.room,
                state=Booking.NEW,
        ...
        if existing_bookings:
            messages.error(
                request, "La habitación tiene reservaciones en ese rango de fechas"
            )
            return redirect(f"/booking/{pk}/edit")
