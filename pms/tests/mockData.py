...
from pms.models import Room_type, Booking, Room
...
class MockData:
    ...
    @staticmethod
    def start_mock_data_for_test():
        type_room = Room_type.objects.create(name="Doble", price=20, max_guests=2)
        r1 = Room.objects.create(room_type=type_room, name='1.1', description='UNA HAB')
        r2 = Room.objects.create(room_type=type_room, name='1.2', description='DOS HAB')
        r3 = Room.objects.create(room_type=type_room, name='2.1', description='HAB')
        r4 = Room.objects.create(room_type=type_room, name='11', description='HAB 11')
        b1 = Booking.objects.create(checkin=datetime.today(),
                                    checkout=datetime.today() + timedelta(days=5), room=r1, guests=1, total=20,
                                    code=get())
