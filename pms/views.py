...
class DashboardView(View):
    def get(self,request):
        ...
        #get  occupation percent
        all_rooms = Room.objects.all().count()
        occupation = new_bookings / all_rooms
...
class RoomsSearchedView(View):
    @method_decorator(ensure_csrf_cookie)
    def get(self,request):
        #renders a list of rooms searched
        querie =request.GET.dict()
        name_room = querie['filter_room']
        rooms = Room.objects.filter(name__startswith=name_room)
        context = {
            'rooms':rooms
        }
        return render(request,"rooms.html",context)
...
#This function should be in other folder like services, but for the character about the test I implemented here.
